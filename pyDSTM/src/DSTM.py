...
class DSTM(object):
    ...
    def until(self):
        return (self.cyclecount < self.stopCycle)

    def limitlessRun(self):
        self.run(self.forever)
    
    def limitedRun(self,maxi):
        self.stopCycle = maxi
        return self.run(self.until)

    def run(self,truth):
        map(lambda x: x.start(),self.reader)
        self.writer.start()
        self.observations = list()
        while(truth()):
            time.sleep(self.cycletime)
            backup = self.data.copy()
            self.debug('Start cycle in state: ' + self.state)
            trigger = self.getTrigger()
            rules = filter(lambda l: (l.prev == self.state) and self.matchTriggers(trigger,l.trig),self.behav)
            rules = filter(lambda l: self.applyCondition(l.cond),rules)
            trname = ''
            if (len(rules) > 0):
                rule = random.choice(rules)
                trname = rule.name
                self.state = rule.next
                self.applyActions(rule.acts)
                self.debug('Trigger received: ' + rule.trig)
                self.debug('Condition ok: ' + rule.cond)
                self.debug('Next state: ' + rule.next)
                self.debug('Action applied: ' + rule.acts)
            self.record(trname)
            self.recordStat()
            self.checkEvents(backup)
            self.cyclecount += 1
            self.cleanChannels()
